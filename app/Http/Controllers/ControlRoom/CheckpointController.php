...
{
    /**
     * List checkpoints (optionally filtered by site)
     */
    public function index(Request $request)
    {
        ...
        $checkpoints = $query->orderBy('name')->get()->map(function ($cp) {
            return [
                'id' => $cp->id,
                'name' => $cp->name,
                'code' => $cp->code,
        ...
        return Inertia::render('ControlRoom/Checkpoints/Index', [
            'checkpoints' => $checkpoints,
            'sites' => $sites,
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'scan_radius_meters' => 'nullable|integer|min:1|max:1000',
        ...
        $checkpoint = Checkpoint::create([
            ...
            'requires_photo' => $validated['requires_photo'] ?? false,
            'scan_radius_meters' => $validated['scan_radius_meters'] ?? 50,
            'latitude' => $validated['latitude'] ?? null,
            'longitude' => $validated['longitude'] ?? null,
        ]);

        return redirect()->back()->with('success', "Checkpoint \"{$checkpoint->name}\" created with code: {$checkpoint->code}");
    }
    ...
    public function update(Request $request, Checkpoint $checkpoint)
    {
        ...
        return redirect()->back()->with('success', "Checkpoint \"{$checkpoint->name}\" updated successfully");
    }
    ...
    public function destroy(Checkpoint $checkpoint)
    {
        ...
        return redirect()->back()->with('success', "Checkpoint \"{$name}\" deleted successfully");
    }
...
HTML;
    ...
    /**
     * Download all checkpoint QR codes as ZIP
     */
    public function downloadBulk()
    {
        ...
        $checkpoints = Checkpoint::with(['clientSite.client'])->get();
        ...
        return new StreamedResponse(function() use ($tmpFile) {
            ...
            'Content-Disposition' => 'attachment; filename="checkpoint_qr_codes_' . now()->format('Ymd_His') . '.zip"',
