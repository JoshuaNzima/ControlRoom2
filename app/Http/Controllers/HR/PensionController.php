...
use App\Models\HR\HrPensionScheme;
...
class PensionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $enrollments = HrPensionEnrollment::with(['scheme:id,name','guard:id,name,employee_id'])
            ->when($filterSchemeId > 0, fn($q) => $q->where('hr_pension_scheme_id', $filterSchemeId))
        ...
        return Inertia::render('HR/Pensions', [
            'schemes' => $schemes,
    ...
    public function storeScheme(Request $request)
    {
        ...
        HrPensionScheme::create($data);
        return back()->with('success', 'Scheme created');
    }

    public function updateScheme(Request $request, HrPensionScheme $scheme)
    {
        $data = $request->validate([
    ...
    public function destroyScheme(HrPensionScheme $scheme)
    {
        $scheme->delete();
    ...
    {
        $data = $request->validate([
            'hr_pension_scheme_id' => ['required','exists:hr_pension_schemes,id'],
        ...
        $data['created_by'] = optional($request->user())->id;
        $enrollment = HrPensionEnrollment::create($data);

        // Notify HR managers of new pension enrollment
        try {
            ...
            if ($recipients->isNotEmpty()) {
                $enrollment->loadMissing(['scheme:id,name','guard:id,name,employee_id']);
                $payload = [
                    'title' => 'New Pension Enrollment',
                    'message' => sprintf('%s enrolled to %s', optional($enrollment->guard)->name ?? 'Guard', optional($enrollment->scheme)->name ?? 'Scheme'),
                    'url' => route('hr.pensions.index'),
                ];
    ...
    public function updateEnrollment(Request $request, HrPensionEnrollment $enrollment)
    {
        $data = $request->validate([
            'start_date' => ['nullable','date'],
            'end_date' => ['nullable','date','after_or_equal:start_date'],
        ...
        // Notify HR managers of enrollment update
        try {
            $recipients = User::permission('hr.employees.manage')->get();
            if ($recipients->isEmpty()) {
                $recipients = User::role('super_admin')->get();
            ...
            if ($recipients->isNotEmpty()) {
                $enrollment->loadMissing(['scheme:id,name','guard:id,name,employee_id']);
                $payload = [
                    'title' => 'Pension Enrollment Updated',
                    'message' => sprintf('%s enrollment on %s updated', optional($enrollment->guard)->name ?? 'Guard', optional($enrollment->scheme)->name ?? 'Scheme'),
                    'url' => route('hr.pensions.index'),
                ];
                Notification::send($recipients, new GenericDbNotification($payload));
            }
    ...
    public function destroyEnrollment(HrPensionEnrollment $enrollment)
    {
        $enrollment->delete();
    ...
    public function enrollmentsExport(Request $request)
    {
        ...
        $items = HrPensionEnrollment::with(['scheme:id,name','guard:id,name,employee_id'])
            ->when($filterSchemeId > 0, fn($q) => $q->where('hr_pension_scheme_id', $filterSchemeId))
            ->when($memberStatus !== '', fn($q) => $q->where('status', $memberStatus))
            ->when($memberSearch !== '', function ($q) use ($memberSearch) {
                $q->whereHas('guard', function ($qq) use ($memberSearch) {
        ...
        $filename = 'pension_enrollments_' . now()->format('Ymd_His') . '.csv';
        $rows = [];
