...
export default function CalendarModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const [duties, setDuties] = useState<OfficeDuty[]>([]);
  ...
  const fetchDuties = async () => {
    ...
    try {
      const response = await axios.get('/front-office/api/office-duties', {
        params: { duty_type: 'calendar_schedule' }
      });
      setDuties(response.data.data || []);
    } catch (error) {
      console.error('Failed to fetch calendar duties:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-900">
        ...
        <div className="space-y-4 mt-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="e.g., Board Meeting with CEO"
                      required
                      className="dark:bg-gray-800 dark:border-gray-700"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="recipient">Recipient/Attendees</Label>
                    <Input
                      id="recipient"
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Meeting agenda, location details, preparation notes..."
                    rows={3}
                    className="dark:bg-gray-800 dark:border-gray-700"
                  />
                </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              <IconMapper name="Calendar" size={48} className="mx-auto mb-4 opacity-50" />
              <p>No calendar events yet.</p>
              <p className="text-sm mt-1">Click "Add Event" to create your first schedule item.</p>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        <span className={`px-2 py-0.5 text-xs rounded-full ${getStatusColor(duty.status)}`}>
