...
export default function DispatchesIndex({ auth = {}, openDispatches = [], recentDispatches = [], vehicles = [], drivers = [], sites = [] }: Props) {
              ...
              {openDispatches.length ? openDispatches.map((d) => (
                <div key={d.id} className="p-3 rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700">
                  ...
                  <div className="mt-3 flex justify-end">
                    <button onClick={() => setReturnOpen({ open: true, id: d.id })} className="px-3 py-1.5 text-xs rounded-md bg-green-600 text-white hover:bg-green-700">Record Return</button>
                  </div>
                </div>
              )) : (
...
function CreateDispatchModal({ open, onClose, vehicles, drivers, sites }: { open: boolean; onClose: () => void; vehicles: VehicleOpt[]; drivers: DriverOpt[]; sites: SiteOpt[]; }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('control-room.dispatches.store'), { onSuccess: () => { reset(); onClose(); } });
  };
      ...
      <div className="px-6 py-4 bg-white dark:bg-gray-900">
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.vehicle_id as any} onChange={(e) => setData('vehicle_id', e.target.value ? Number(e.target.value) : '' as any)} required>
          ...
          <div>
            ...
            <select className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.driver_id as any} onChange={(e) => setData('driver_id', e.target.value ? Number(e.target.value) : '' as any)} required>
          ...
          <div>
            ...
            <select className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.origin_site_id as any} onChange={(e) => setData('origin_site_id', e.target.value ? Number(e.target.value) : '' as any)}>
              <option value="">—</option>
          ...
          <div>
            ...
            <select className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.destination_site_id as any} onChange={(e) => setData('destination_site_id', e.target.value ? Number(e.target.value) : '' as any)}>
              <option value="">—</option>
              {sites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          </div>
          <div>
            ...
            <input type="number" className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.odometer_out as any} onChange={(e) => setData('odometer_out', e.target.value ? Number(e.target.value) : '' as any)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fuel Level Out (%)</label>
            <input type="number" min={0} max={100} className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.fuel_level_out as any} onChange={(e) => setData('fuel_level_out', e.target.value ? Number(e.target.value) : '' as any)} />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Purpose</label>
            <input className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.purpose} onChange={(e) => setData('purpose', e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes</label>
            <textarea rows={3} className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.notes_out} onChange={(e) => setData('notes_out', e.target.value)} />
          </div>
...
function ReturnDispatchModal({ open, onClose, id }: { open: boolean; onClose: () => void; id: number|null }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    post(route('control-room.dispatches.return', id), { onSuccess: onClose });
  };
  return (
      ...
      <div className="px-6 py-4 bg-white dark:bg-gray-900">
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.odometer_in as any} onChange={(e) => setData('odometer_in', e.target.value ? Number(e.target.value) : '' as any)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fuel Level In (%)</label>
            <input type="number" min={0} max={100} className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.fuel_level_in as any} onChange={(e) => setData('fuel_level_in', e.target.value ? Number(e.target.value) : '' as any)} />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes</label>
            <textarea rows={3} className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.notes_in} onChange={(e) => setData('notes_in', e.target.value)} />
          </div>
          <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm rounded-md bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-100 hover:bg-gray-300 dark:hover:bg-gray-600" disabled={processing}>Cancel</button>
            <button type="submit" disabled={processing} className="px-4 py-2 text-sm rounded-md bg-green-600 text-white hover:bg-green-700 disabled:bg-gray-400">Save</button>
          </div>
        </form>
      </div>
    </Modal>
