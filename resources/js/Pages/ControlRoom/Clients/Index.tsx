...
export default function ClientsIndex() {
  const { clients, filters = {}, stats } = usePage().props as any;
  ...
  const reset = () => {
    setSearch('');
    setStatus('');
    router.get(route('control-room.clients'), {}, { preserveState: true, preserveScroll: true });
  };

  const statCards = [
    { label: 'Total Clients', value: stats?.total || 0, icon: 'Building', color: 'blue' },
    { label: 'Active', value: stats?.active || 0, icon: 'CheckCircle', color: 'emerald' },
    { label: 'Inactive', value: stats?.inactive || 0, icon: 'XCircle', color: 'gray' },
    { label: 'With Sites', value: stats?.with_sites || 0, icon: 'MapPin', color: 'amber' },
  ];
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <Link
                  href={route('control-room.clients.show', { client: 'create' })}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-white text-red-900 rounded-lg hover:bg-red-50 font-medium text-sm"
                >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Link
                            ...
                            className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
                          >
                            <IconMapper name="Eye" size={14} />
                            View
                          </Link>
                        </div>
                      </div>
