...
const AgentStatusMap: React.FC<Props> = ({ agents = [], onAgentClick = () => {} }) => {
	...
	useEffect(() => {
		...
		(agents || []).forEach(agent => {
			if (agent.latitude && agent.longitude) {
				...
				if (markersRef.current[idKey]) {
					// Update existing marker position
					markersRef.current[idKey].setLatLng([agent.latitude, agent.longitude]);
				} else {
					// Create new marker
				...
				const popup = markersRef.current[String(agent.id)].getPopup();
				popup.setContent(`
					<div class="text-sm">
						<strong>${agent.name}</strong><br>
						Status: ${agent.status}<br>
