...
interface OperationsDashboardProps {
  ...
  qrScanSummary?: QrScanData;
  auth?: { user?: { name?: string } };
}

export default function OperationsDashboard({ stats, sites, deployments, personnel, recentIncidents, escalatedIncidents = [], escalatedDowns = [], recentScans, qrScanSummary: initialQrData, auth }: OperationsDashboardProps) {
  ...
  return (
    <AuthenticatedLayout header="Operations Dashboard" user={auth?.user as User | undefined}>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
          ...
          <div className="relative p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6">
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="p-3 sm:p-4 bg-white/10 rounded-xl backdrop-blur-sm"><IconMapper name="MapPin" size={28} className="sm:w-8 sm:h-8" /></div>
                <div><h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">Field Operations</h1><p className="text-emerald-100 mt-0.5 sm:mt-1 text-sm sm:text-base">Site coverage, deployments & field personnel</p></div>
              </div>
              <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                <div className="px-3 sm:px-4 py-2 bg-white/10 rounded-lg backdrop-blur-sm"><p className="text-[10px] sm:text-xs text-emerald-200">System Time</p><p className="text-base sm:text-lg font-mono font-semibold">{currentTime.toLocaleTimeString('en-US', { hour12: false })}</p></div>
                <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/10 rounded-lg backdrop-blur-sm border border-white/20">
                  ...
                  <div><p className="text-[10px] sm:text-xs text-emerald-200">Field Status</p><p className="text-xs sm:text-sm font-semibold">{safeStats.understaffedSites === 0 ? 'All Sites Covered' : `${safeStats.understaffedSites} Understaffed`}</p></div>
                </div>
        ...
        <div className="flex gap-1.5 sm:gap-2 p-1 bg-gray-100 dark:bg-gray-800 rounded-xl w-fit flex-wrap">
          {[{id:'overview',label:'Overview',icon:'LayoutGrid'},{id:'sites',label:'Site Status',icon:'Building'},{id:'deployments',label:'Deployments',icon:'Map'},{id:'personnel',label:'Personnel',icon:'Users'},{id:'qr-scans',label:'QR Scans',icon:'ScanLine'}].map(tab=>(
            <button key={tab.id} onClick={()=>setActiveTab(tab.id as typeof activeTab)} className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${activeTab===tab.id?'bg-white dark:bg-gray-700 text-emerald-600 dark:text-emerald-400 shadow-sm':'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-300">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              <StatCard icon={<IconMapper name="AlertTriangle" size={18} className="sm:w-5 sm:h-5" />} title="Incidents" value={safeStats.activeIncidents} subtitle="Field issues today" color={safeStats.activeIncidents === 0 ? 'green' : safeStats.activeIncidents > 5 ? 'red' : 'amber'} />
            </div>
        ...
        {activeTab === 'qr-scans' && (
          <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => fetchQrData(true)} disabled={isRefreshing} className="text-xs text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/30">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              ...
              <Card className="dark:bg-gray-900 dark:border-gray-800">
                ...
                <CardContent className="p-4 sm:p-6 pt-2 sm:pt-4"><div className="space-y-3">
                  {qrData?.recentScans?.slice(0, 10).map(scan => (
                  ...
                  {qrData.week.dailyTrend.map((d, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        ...
                        <div className="w-full bg-emerald-500/80 rounded-t-sm" style={{ height: `${height}%` }} />
