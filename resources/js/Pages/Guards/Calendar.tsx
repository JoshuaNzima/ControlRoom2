  ...
  return (
    <SupervisorLayout title="Attendance Calendar">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => goToMonth(-1)}
              className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200"
            >
              Prev
            </button>
            <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 min-w-[140px] text-center">
              {monthLabel}
            </div>
            <button
              type="button"
        ...
        <div className="grid grid-cols-7 gap-2 text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">
                    ...
                    <div className="space-y-0.5">
                      {Object.entries(day.statuses).map(([status, info]) => (
                        <div key={status} className="flex items-center justify-between gap-1">
                          <span
                            className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium ${statusColors[status] || 'bg-gray-600 text-white'}`}
                          >
                            {status.replace('_', ' ')}
                          </span>
                          <span className="text-[10px] text-gray-800 dark:text-gray-200">
                            ...
                            {info.backdated_count > 0 && (
                              <span className="ml-1 text-[9px] text-yellow-300 dark:text-yellow-300">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow border border-gray-200 dark:border-gray-800 p-4 space-y-3 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {summary && summary.by_status && (Object.entries(summary.by_status) as [string, number][]).map(([status, count]) => (
              <span
                key={status}
                className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[status] || 'bg-gray-600 text-white'}`}
              >
