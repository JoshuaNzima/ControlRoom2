...
export default function Pensions() {
  const { auth, schemes, enrollments, guards, filters }: any = usePage().props;
  ...
  const enrollmentRows: Enrollment[] = useMemo(() => (enrollments?.data || enrollments || []), [enrollments]);

  const doFilter = () => router.get(route('hr.pensions.index'), { scheme_search: schemeSearch, scheme_status: schemeStatus, scheme_id: filterSchemeId, member_status: memberStatus, member_search: memberSearch, per_page: perPage }, { preserveState: true, replace: true });
      ...
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-red-900 dark:text-gray-100">Pension Schemes</h1>
              <p className="text-sm text-red-800/80 dark:text-gray-400 mt-1">Manage pension schemes and guard enrollments.</p>
            </div>
            <div className="flex gap-2 flex-wrap">
              <a href={route('hr.pensions.enrollments.export', { scheme_id: filterSchemeId, member_status: memberStatus, member_search: memberSearch })} className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-white dark:bg-gray-800 text-red-800 dark:text-gray-100 border border-red-200 dark:border-gray-700 hover:bg-red-50 dark:hover:bg-gray-700">Export Enrollments</a>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          ...
                          <td className="px-4 py-2 text-right">
                            <div className="inline-flex gap-2">
                              ...
                              <button onClick={()=>{ if(confirm('Delete scheme?')) router.delete(route('hr.pensions.schemes.destroy', s.id)); }} className="text-red-600 hover:text-red-800">Delete</button>
                            </div>
                          </td>
                        </tr>
                      ))}
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                    {enrollmentRows.map(m => (
                      <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-4 py-2 text-right">
                          <div className="inline-flex gap-2">
                            <Link href={route('hr.pensions.enrollments.update', m.id)} method="put" as="button" data={{ status: m.status === 'active' ? 'inactive' : 'active' }} className="text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100">Toggle</Link>
                            <button onClick={()=>{ if (confirm('Delete enrollment?')) router.delete(route('hr.pensions.enrollments.destroy', m.id)); }} className="text-red-600 hover:text-red-800">Delete</button>
                          </div>
                        </td>
                      </tr>
                    ))}
                ...
                {(enrollments?.meta?.last_page ?? 1) > 1 && (
                  <div className="bg-gray-50 dark:bg-gray-800 px-4 py-3 flex justify-center gap-2">
                    {(enrollments?.links ?? []).map((link: any, idx: number) => (
                      <Link key={idx} href={link.url || '#'} className={`px-3 py-1 rounded text-xs ${link.active ? 'bg-red-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800'}`} dangerouslySetInnerHTML={{ __html: link.label }} />
                    ))}
                  </div>
                )}
...
function SchemeModal({ open, onClose, scheme }: { open: boolean; onClose: () => void; scheme?: Scheme }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (scheme) {
      put(route('hr.pensions.schemes.update', scheme.id), { onSuccess: () => { reset(); onClose(); } });
    } else {
      post(route('hr.pensions.schemes.store'), { onSuccess: () => { reset(); onClose(); } });
    }
  };
...
function EnrollModal({ open, onClose, guards, schemes }: { open: boolean; onClose: () => void; guards: GuardOpt[]; schemes: Scheme[] }) {
  const { data, setData, post, processing, reset, errors } = useForm<{ hr_pension_scheme_id: number|''; guard_id: number|''; start_date: string; end_date: string; member_no: string; status: string }>({
    hr_pension_scheme_id: '' as any,
    guard_id: '' as any,
    start_date: '',
    end_date: '',
    member_no: '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('hr.pensions.enrollments.store'), { onSuccess: () => { reset(); onClose(); } });
  };
      ...
      <div className="fixed inset-x-0 bottom-0 md:inset-0 md:m-auto md:max-w-xl bg-white dark:bg-gray-900 rounded-t-2xl md:rounded-2xl shadow-lg">
        ...
        <form onSubmit={submit} className="px-6 py-4 grid grid-cols-1 gap-3">
          <div>
            ...
            <select required className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.hr_pension_scheme_id as any} onChange={(e) => setData('hr_pension_scheme_id', e.target.value ? Number(e.target.value) : '' as any)}>
              <option value="">Select scheme…</option>
              {schemes.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
            {errors.hr_pension_scheme_id && <p className="text-xs text-red-600 mt-1">{errors.hr_pension_scheme_id}</p>}
          </div>
          <div>
