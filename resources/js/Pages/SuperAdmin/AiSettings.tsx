...
export default function AiSettings() {
  const { auth, providers, activeProvider } = usePage<PageProps>().props as any;
  ...
  const enableProvider = (provider: AiProvider) => {
    if (!provider.api_key && !forms[provider.provider]?.api_key) {
      toast({ title: 'API key required', description: 'Please configure the API key first', variant: 'destructive' });
      return;
    }
        ...
        <div className="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-red-700 via-red-600 to-rose-600 text-white shadow-xl">
          ...
          <div className="relative p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">AI Provider Settings</h1>
                <p className="text-red-100 mt-1 text-sm sm:text-base">Configure AI providers for the chatbot assistant</p>
              </div>
            </div>
          </div>
        </div>
