        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => setCreateOpen(true)} className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700">
                New Event
              </button>
                      ...
                      <tr key={ev.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                        ...
                        <td className="px-3 py-2 text-right">
                          <div className="inline-flex items-center gap-2">
                            ...
                            <button type="button" onClick={() => handleDelete(ev)} className="text-red-600 hover:text-red-800">Delete</button>
                          </div>
                        </td>
                      </tr>
                    ))
              ...
              <div className="bg-gray-50 dark:bg-gray-800 px-4 py-3 flex justify-center gap-2">
                {(events?.links ?? []).map((link: any, idx: number) => (
                  <Link
                    key={idx}
                    href={link.url || '#'}
                    className={`px-3 py-1 rounded text-xs ${link.active ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 dark:bg-gray-900 dark:text-gray-100 dark:border-gray-700 dark:hover:bg-gray-800'}`}
      ...
      <div className="px-6 py-4 bg-white dark:bg-gray-900">
        <form className="grid grid-cols-1 sm:grid-cols-2 gap-4" onSubmit={submit}>
          <div>
            ...
            <select className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.client_id} onChange={(e) => setData('client_id', e.target.value ? Number(e.target.value) : ('' as any))}>
          ...
          <div>
            ...
            <input type="date" className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.event_date} onChange={(e) => setData('event_date', e.target.value)} />
          ...
          <div className="sm:col-span-2">
            ...
            <input className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.location} onChange={(e) => setData('location', e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <select className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.billing_type} onChange={(e) => setData('billing_type', e.target.value)}>
          ...
          <div>
            ...
            <input type="number" min={0} step="0.01" className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.rate} onChange={(e) => setData('rate', e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Quantity</label>
            <input type="number" min={1} className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.quantity} onChange={(e) => setData('quantity', Number(e.target.value) || 1)} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min={0} className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.k9_units} onChange={(e) => setData('k9_units', e.target.value)} />
          </div>
          <div className="sm:col-span-2">
          ...
          <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={processing} className="px-4 py-2 text-sm rounded-md bg-indigo-600 text-white hover:bg-indigo-700 disabled:bg-gray-400">{processing ? 'Saving...' : 'Create Event'}</button>
          </div>
        </form>
      </div>
    </Modal>
    ...
    <Modal show={open} onClose={handleClose} maxWidth="2xl">
      ...
      <div className="px-6 py-4 bg-white dark:bg-gray-900">
        <form className="grid grid-cols-1 sm:grid-cols-2 gap-4" onSubmit={submit}>
          ...
          <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
            <button type="button" onClick={handleClose} className="px-4 py-2 text-sm rounded-md bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-700" disabled={processing}>Cancel</button>
            <button type="submit" disabled={processing} className="px-4 py-2 text-sm rounded-md bg-indigo-600 text-white hover:bg-indigo-700 disabled:bg-gray-400">{processing ? 'Saving...' : 'Save'}</button>
          </div>
        </form>
      </div>
    </Modal>
