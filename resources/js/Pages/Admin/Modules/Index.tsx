      ...
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-100">Modules</h1>
          </div>
            ...
            {modules.map((m) => (
              <Link key={m.name} href={route('admin.modules.summary', m.name)} className={`rounded-2xl border p-4 transition hover:shadow ${m.is_active ? 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900' : 'border-dashed border-gray-300 dark:border-gray-700 bg-white/50 dark:bg-gray-900/50'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${m.is_active ? 'bg-coin-100 text-coin-700 dark:bg-coin-900/20 dark:text-coin-200' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'}`}>
