...
const ticketPriorityBadgeClassName = (priority: string) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search title or number" className={frontDeskFieldClassName} />
              <select value={status} onChange={(e) => setStatus(e.target.value)} className={frontDeskFieldClassName}>
                <option value="">All Status</option>
                {(options?.statuses || []).map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <select value={priority} onChange={(e) => setPriority(e.target.value)} className={frontDeskFieldClassName}>
              ...
              <div className="md:col-span-2 flex flex-col sm:flex-row gap-2">
                ...
                <button
                  onClick={() => { setStatus(''); setPriority(''); setSearch(''); router.get(route('admin.front-desk.tickets.index')); }}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-[980px] w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800">
                  <tr>
                    <th className="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">#</th>
                    <th className="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Title</th>
                    ...
                    <th className="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Priority</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                  {tickets?.data?.length ? tickets.data.map(t => (
                    <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="px-4 py-2"><span className={`px-2 py-1 rounded text-xs ${ticketPriorityBadgeClassName(t.priority)}`}>{t.priority}</span></td>
                      ...
                      <td className="px-4 py-2 text-right">
                        <div className="inline-flex gap-2">
                          ...
                          <button
                            onClick={() => { if (confirm('Delete ticket?')) router.delete(route('admin.front-desk.tickets.destroy', t.id)); }}
      ...
      <div className="px-6 py-4 bg-white dark:bg-gray-900">
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            ...
            <input className={frontDeskFieldClassName} value={data.title} onChange={(e) => setData('title', e.target.value)} />
            {errors.title && <p className="text-sm text-red-600">{errors.title}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category</label>
            <select className={frontDeskFieldClassName} value={data.category} onChange={(e) => setData('category', e.target.value)}>
              {categories.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Priority</label>
            <select className={frontDeskFieldClassName} value={data.priority} onChange={(e) => setData('priority', e.target.value)}>
          ...
          <div>
            ...
            <select className={frontDeskFieldClassName} value={data.assigned_to as any} onChange={(e) => setData('assigned_to', e.target.value ? Number(e.target.value) : '' as any)}>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea className={frontDeskFieldClassName} rows={4} value={data.description} onChange={(e) => setData('description', e.target.value)} />
          </div>
