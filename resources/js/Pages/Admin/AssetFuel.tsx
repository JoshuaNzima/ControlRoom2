...
export default function AssetFuel() {
  ...
  const doFilter = () => router.get(route('assets.fuel.index'), { vehicle_id: vehicleId, perPage }, { preserveState: true, replace: true });
      ...
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-red-900 dark:text-gray-100">Fuel Logs</h1>
              <p className="text-sm text-red-800/80 dark:text-gray-400 mt-1">Record fuel consumption per vehicle.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <a href={route('assets.fuel.export', { vehicle_id: vehicleId })} className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-white dark:bg-gray-800 text-red-800 dark:text-gray-100 border border-red-200 dark:border-gray-700 hover:bg-red-50 dark:hover:bg-gray-700">Export CSV</a>
              <button
            ...
            <table className="min-w-[860px] w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800">
                <tr>
                  ...
                  <th className="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Liters</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                {rows.map((r: any) => (
                  <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{r.liters}</td>
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{r.cost ?? '-'}</td>
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{r.odometer ?? '-'}</td>
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{r.vendor || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function CreateFuelModal({ open, onClose, vehicles }: { open: boolean; onClose: () => void; vehicles: any[] }) {
  const { data, setData, post, processing, reset, errors } = useForm<{ vehicle_id: number|''; date: string; liters: string; cost: string; odometer: string; vendor: string }>({
    vehicle_id: '' as any,
    date: '',
    liters: '',
    cost: '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('assets.fuel.store'), { onSuccess: () => { reset(); onClose(); } });
  };
    ...
    <div className={`fixed inset-0 z-50 ${open ? 'block' : 'hidden'}`}>
      ...
      <div className="fixed inset-x-0 bottom-0 md:inset-0 md:m-auto md:max-w-xl bg-white dark:bg-gray-900 rounded-t-2xl md:rounded-2xl shadow-lg">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <div className="text-lg font-semibold text-gray-900 dark:text-gray-100">New Fuel Log</div>
        ...
        <form onSubmit={submit} className="px-6 py-4 grid grid-cols-1 gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" step="0.01" required className={assetFieldClassName} value={data.liters} onChange={(e) => setData('liters', e.target.value)} />
            </div>
            <div>
