        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setCreateOpen(true)} className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-red-600 text-white hover:bg-red-700">New Visitor</button>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, company, badge" className="px-3 py-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" />
              ...
              <div className="md:col-span-2 flex gap-2">
                ...
                <button onClick={() => { setStatus(''); setSearch(''); router.get(route('admin.front-desk.visitors.index')); }} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded">Reset</button>
              </div>
            </div>
          </div>
                    ...
                    <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="px-4 py-2 text-right">
                        <div className="inline-flex gap-2">
                          ...
                          <button onClick={() => { if (confirm('Delete visitor?')) router.delete(route('admin.front-desk.visitors.destroy', v.id)); }} className="text-red-600 hover:text-red-800">Delete</button>
                        </div>
                      </td>
                    </tr>
                  )) : (
      ...
      <div className="px-6 py-4 bg-white dark:bg-gray-900">
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.company} onChange={(e) => setData('company', e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Contact Person</label>
            <input className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.contact_person} onChange={(e) => setData('contact_person', e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Badge Number</label>
            <input className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" value={data.badge_number} onChange={(e) => setData('badge_number', e.target.value)} />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea className="w-full border rounded-md p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100" rows={3} value={data.purpose} onChange={(e) => setData('purpose', e.target.value)} />
          </div>
