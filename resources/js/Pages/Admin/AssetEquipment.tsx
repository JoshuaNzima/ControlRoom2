...
export default function AssetEquipment({ auth = {}, equipment, options }: Props) {
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="px-4 py-2 text-right">
                        <div className="inline-flex gap-2">
                          ...
                          <button onClick={() => { if (confirm('Delete equipment?')) router.delete(route('admin.assets.equipment.destroy', item.id)); }} className="text-red-600 hover:text-red-800">Delete</button>
                        </div>
                      </td>
                    </tr>
                  )) : (
...
function EquipmentModal({ open, onClose, statuses, users, equipment }: { open: boolean; onClose: () => void; statuses: string[]; users: UserOpt[]; equipment?: Equipment }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (equipment) {
      put(route('admin.assets.equipment.update', equipment.id), { onSuccess: () => { reset(); onClose(); } });
    } else {
      post(route('admin.assets.equipment.store'), { onSuccess: () => { reset(); onClose(); } });
    }
  };
