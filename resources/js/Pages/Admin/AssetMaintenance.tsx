...
export default function AssetMaintenance() {
  ...
  const doFilter = () => router.get(route('assets.maintenance.index'), { vehicle_id: vehicleId, perPage }, { preserveState: true, replace: true });
  const reset = () => { setVehicleId(0); setPerPage(15); router.get(route('assets.maintenance.index'), {}, { preserveState: false }); };

  return (
    <AssetManagementLayout title="Maintenance" user={auth?.user as any}>
      <Head title="Maintenance" />
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-red-900 dark:text-gray-100">Vehicle Maintenance</h1>
              <p className="text-sm text-red-800/80 dark:text-gray-400 mt-1">Record maintenance events and downtime.</p>
            </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                {rows.map((r: any) => (
                  <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{r.type || '-'}</td>
                    ...
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{r.down_time_hours ?? '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <CreateMaintenanceModal open={openCreate} onClose={() => setOpenCreate(false)} vehicles={vehicles || []} />
        </div>
      </div>
...
function CreateMaintenanceModal({ open, onClose, vehicles }: { open: boolean; onClose: () => void; vehicles: any[] }) {
  const { data, setData, post, processing, reset, errors } = useForm<{ vehicle_id: number|''; date: string; type: string; vendor: string; cost: string; down_time_hours: string; notes: string }>({
    vehicle_id: '' as any,
    date: '',
    type: '',
    vendor: '',
