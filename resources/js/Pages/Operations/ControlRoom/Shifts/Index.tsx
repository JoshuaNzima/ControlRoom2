    ...
    <ControlRoomLayout title="Shift Management">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link href={route('operations.control-room.shifts.create')} className="inline-flex items-center px-3 py-2 rounded-md bg-indigo-600 text-white text-sm">Create Shift</Link>
        </div>
              ...
              {shifts?.data?.map((s: any) => (
                <div key={s.id} className="py-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Link href={route('operations.control-room.shifts.show', s.id)} className="text-sm text-indigo-600">View</Link>
                    <Link href={route('operations.control-room.shifts.edit', s.id)} className="text-sm text-gray-600">Edit</Link>
                  </div>
                </div>
              ))}
            </div>
