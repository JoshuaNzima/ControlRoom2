...
const Zones = ({ auth, zones: zonesProp, commanders = [], sites = [] }: ZonesProps) => {
  const { props } = usePage();
  // Normalize zones prop which can be server-side paginated object or fallback to page props
  const zonesMeta: { data: ZoneItem[]; current_page: number; last_page: number } = (zonesProp && (zonesProp as any).data)
    ? (zonesProp as any)
    : ((props as any).zones ?? { data: (zonesProp as any) ?? [], current_page: 1, last_page: 1 });
  const zonesList: ZoneItem[] = Array.isArray(zonesProp) && zonesProp.length ? (zonesProp as any) : zonesMeta.data || [];
  const [isLoading, setIsLoading] = React.useState<boolean>(false);
  const [open, setOpen] = React.useState<boolean>(false);
  const [editing, setEditing] = React.useState<ZoneItem | null>(null);

  // Fallback: if server didn't provide `sites`, fetch them client-side
  React.useEffect(() => {
    const fetchSites = async () => {
      if (sites && sites.length > 0) return;
      try {
        let url = '/sites';
        try { if (typeof (route) === 'function') url = route('sites.index'); } catch (e) { /* fallback */ }
        const res = await axios.get(url, { headers: { Accept: 'application/json' }, withCredentials: true });
        const data = res.data ?? {};
        // If response wrapped (Inertia) try to extract
        const items = data.data ?? data.sites ?? data;
        // if items is object with data property
        if (Array.isArray(items)) {
          // We can't set the `sites` prop directly, but we can populate form choices if needed by keeping a local copy.
          // For now, setData site list if empty (so form shows items)
          // Note: Zones form uses `sites` directly; we will shallowly patch the DOM by setting a window variable that Zones form can read.
          (window as any).__fetched_sites = items;
        }
      } catch (err) {
        // ignore — leave sites empty
        console.warn('Could not fetch sites fallback', err);
      }
    };
      ...
      <div className="space-y-6">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{zonesList.length}</p>
                </div>
          ...
          <Card className="dark:bg-gray-800 dark:border-gray-700">
            <CardContent className="p-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600 dark:text-green-400">{zonesList.filter((z: ZoneItem) => z.status === 'active').length}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{zonesList.reduce((sum: number, z: ZoneItem) => sum + (z.active_guard_count ?? 0), 0)}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{zonesList.reduce((sum: number, z: ZoneItem) => sum + (z.sites_count ?? 0), 0)}</p>
                </div>
        ...
        <Card className="dark:bg-gray-800 dark:border-gray-700">
          ...
          <CardContent>
            <ScrollableList
              onLoadMore={() => {
                if (zonesMeta.current_page < zonesMeta.last_page) {
                  setIsLoading(true);
                  router.get(
                    route('control-room.zones.index'),
                    { page: zonesMeta.current_page + 1 },
                    { preserveState: true, preserveScroll: true, onFinish: () => setIsLoading(false) }
                  );
                }
              }}
              isLoading={isLoading}
              hasMore={zonesMeta.current_page < zonesMeta.last_page}
              containerClassName="pr-2"
            >
              {zonesList.map((zone: ZoneItem) => (
