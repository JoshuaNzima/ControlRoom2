...
const CoverageTrendChart = React.lazy(() => import('@/Components/Operations/ControlRoom/CoverageTrendChart'));
const AttendanceChart = React.lazy(() => import('@/Components/Operations/ControlRoom/AttendanceChart'));
import {
    Chart as ChartJS,
    CategoryScale,
    LinearScale,
...
// Small client-side incidents infinite list component
function IncidentsInfinite() {
    ...
    const load = async (p = 1) => {
        ...
        setLoading(true);
        try {
            let url = '/control-room/downs';
            try { if (typeof (route) === 'function') url = route('control-room.downs.index'); } catch(e) { /* fallback */ }

            const res = await axios.get(url, { params: { page: p }, headers: { Accept: 'application/json' }, withCredentials: true });
            const data = res.data ?? {};

            // Paginator: data.data (items) and data.meta for pages
            const items: any[] = data.data ?? (Array.isArray(data) ? data : []);
            const meta: any = data.meta ?? data;

            if (p === 1) setIncidents(items as RecentIncident[]);
            else setIncidents(prev => [...prev, ...(items as RecentIncident[])]);
    ...
    return (
        <InfiniteScroll onIntersect={() => load(page + 1)} isLoading={loading} hasMore={!lastPage}>
                    ...
                    <div key={incident.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        ...
                        <div className="text-right">
                            ...
                            {incident.escalation_level > 0 && (
                                <div className="text-xs text-red-600 dark:text-red-400 mt-1">Escalation Level {incident.escalation_level}</div>
                            )}
                        </div>
                    </div>
                )) : (
                    <div className="text-center py-6 text-gray-500 dark:text-gray-400">No recent incidents</div>
                )}
            </div>
        </InfiniteScroll>
    );
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card className="dark:bg-gray-800 dark:border-gray-700">
                        ...
                        {/* Recent Incidents (infinite scroll) */}
                    <Card className="dark:bg-gray-800 dark:border-gray-700">
                        <CardHeader className="flex flex-row items-center justify-between">
