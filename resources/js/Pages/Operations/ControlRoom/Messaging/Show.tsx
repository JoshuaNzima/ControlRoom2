...
const Show = ({ auth, conversation }: ShowProps) => {
    ...
    useEffect(() => {
        ...
        const echo = (window as any).Echo ?? new Echo({
            broadcaster: 'pusher',
        ...
        const channel = (echo as any).join ? (echo as any).join(`conversation.${conversation.id}`) : (echo as any).private ? (echo as any).private(`conversation.${conversation.id}`) : null;
        ...
        return () => {
            try {
                if ((echo as any).leave) (echo as any).leave(`conversation.${conversation.id}`);
            } catch (err) {
                // ignore
            }
        };
    ...
    const sendMessage = async (e: React.FormEvent) => {
        ...
        try {
            const { data } = await axios.post(
                route('operations.control-room.messaging.messages.send', conversation.id),
                {
                    content: newMessage,
