...
export default function ZoneAssign() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('operations.control-room.zones.assignments.store', zone.id), { onSuccess: () => reset() });
  };
        ...
        <Card className="dark:bg-gray-800 dark:border-gray-700">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Link href={route('operations.control-room.zones.index')} className="text-sm text-indigo-600">Back to Zones</Link>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div className="space-y-2">
              ...
              {assignments.map((a: any) => (
                <div key={a.id} className="flex items-center justify-between p-3 border rounded-md dark:border-gray-600">
                  ...
                  <Link href={route('operations.control-room.zones.assignments.destroy', [zone.id, a.id])} method="delete" as="button" className="text-sm text-red-600">Unassign</Link>
                </div>
              ))}
            </div>
          </CardContent>
