...
export default function About() {
  return (
    <PublicLayout title="About — Coin Security">
      ...
      {/* Stats bar */}
      <section className="relative py-12 bg-coin-surface border-b border-coin-border overflow-hidden">
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((s, i) => (
              <AnimatedSection key={s.label} delay={i * 0.08}>
                <div className="text-center">
                  <div className="text-3xl md:text-4xl font-bold text-coin-accent">{s.value}</div>
      ...
      <section className="relative py-16 bg-coin-surface overflow-hidden">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-coin-accent/10 to-transparent animate-scan-line" />
        </div>
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v, i) => (
              <AnimatedSection key={v.title} delay={i * 0.08}>
                <div className="rounded-2xl border border-coin-border p-6 bg-coin-card/60 h-full transition-all duration-300 hover:border-coin-accent/30 hover:shadow-lg hover:shadow-coin-glow hover-lift">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[{icon:'Shield',title:'Licensed & Vetted',desc:'All personnel are licensed, background-checked, and continuously trained.'},{icon:'Activity',title:'Operational Excellence',desc:'Clear SLAs, post orders, and digital reporting for accountability.'},{icon:'Headphones',title:'24/7 Support',desc:'Always-on support for incidents, escalations, and service updates.'}].map((item, i) => (
              <AnimatedSection key={i} delay={i * 0.1}>
                <div className="group rounded-2xl border border-coin-border p-6 bg-coin-card/60 transition-all duration-300 hover:border-coin-accent/30 hover:shadow-lg hover:shadow-coin-glow hover-lift">
          ...
          <AnimatedSection delay={0.15}>
            <div className="rounded-2xl border border-coin-border p-8 bg-coin-card/60">
