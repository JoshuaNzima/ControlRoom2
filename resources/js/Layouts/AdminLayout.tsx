...
export default function AdminLayout({ title, children }: Props) {
    ...
    const operationsNav: NavItem[] = [
        ...
        ...(isAssetManager ? ([{ name: 'Assets', href: route('assets.index'), icon: <IconMapper name="Boxes" size={20} />, badge: counters?.assets_handovers_outstanding || undefined }] as NavItem[]) : []),
    ];

    const financeNav: NavItem[] = [
        { name: 'Payments', href: route('admin.payments.index'), icon: <IconMapper name="Wallet" size={20} /> },
        ...(!isAdminUser ? ([{ name: 'Requisitions', href: route('requisitions.index'), icon: <IconMapper name="ClipboardList" size={20} />, badge: counters?.requisitions_my_open }] as NavItem[]) : []),
    ...
    return (
        <div className="min-h-screen bg-red-50 dark:bg-gray-900 overflow-x-hidden">
            ...
            <div className={`fixed top-0 left-0 bottom-0 flex flex-col w-64 bg-red-900 dark:bg-gray-950 text-white transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out z-50`}>
                ...
                <UserSection user={user} roleDisplay={roleDisplay} profileRoute="admin.profile" showSuperAdmin={isSuperAdmin} superAdminRoute="superadmin.dashboard" />
            </div>
